"""앱 전체가 공유하는 단일 시각 기준(KST).

`datetime.now()`와 `date.today()`는 **호스트 타임존**을 따라간다. 이 앱의 "하루"는
항상 KST여야 하는데(장 일정, `/market`의 하루 경계, 보존 기간), 호스트에 맡기면
...
(`news/utils.py`가 중국 소스를 CST로 간주해 KST로 변환한다), 여기 함수들은
...
def ensure_kst(value: datetime) -> datetime:
    """저장된 타임스탬프를 KST aware로 정규화한다.

    오프셋이 없는 값은 KST로 간주한다. 이 앱이 기록한 시각은 언제나 KST이고,
