...
스레드 분리만으로는 Ollama 경합을 막지 못한다. 러너 슬롯이 1개면 요청이
직렬화되고, 리서치 분석처럼 중간에 양보할 수 없는 단일 호출이 먼저 잡으면
주기 뉴스 번역이 그 뒤에서 대기한다. 그래서 스레드 분리와 별개로,
뉴스 주기가 도는 동안에는 비긴급 LLM 작업의 *시작*을 보류시킨다
(`urgent_phase` / `wait_for_urgent_idle`).
"""
